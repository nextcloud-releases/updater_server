Feature: Testing the update scenario of beta releases
  ...
  Scenario: Updating an outdated Nextcloud 16.0.0 on the beta channel
    ...
    When The request is sent
    Then The response is non-empty
    And Update to version "17.0.10.0" is available
    And URL to download is "https://download.nextcloud.com/server/prereleases/nextcloud-17.0.10RC1.zip"
    And URL to documentation is "https://docs.nextcloud.com/server/17/admin_manual/maintenance/upgrade.html"
    And EOL is set to "1"
    And The signature is
    """
    crQjumeNW7rNiCkzFDXa+/q1HqKbN9H6h2QiTD9h+NyxhVNbGhp/0kNKURwoYNqN
  ...
  Scenario: Updating an outdated Nextcloud 18.0.0 on the beta channel
    ...
    When The request is sent
    Then The response is non-empty
    And Update to version "18.0.10.0" is available
    And URL to download is "https://download.nextcloud.com/server/prereleases/nextcloud-18.0.10RC1.zip"
    And URL to documentation is "https://docs.nextcloud.com/server/18/admin_manual/maintenance/upgrade.html"
    And EOL is set to "0"
    And The signature is
    """
    
    """

  Scenario: Updating an outdated Nextcloud 18.0.9.1 on the beta channel
    Given There is a release with channel "beta"
    And The received version is "18.0.9.1"
    And The received PHP version is "7.2.0"
    And the installation mtime is "11"
    When The request is sent
    Then The response is non-empty
    And Update to version "19.0.3.1" is available
    And URL to download is "https://download.nextcloud.com/server/releases/nextcloud-19.0.3.zip"
    And URL to documentation is "https://docs.nextcloud.com/server/19/admin_manual/maintenance/upgrade.html"
    And EOL is set to "0"
    And The signature is
    """
    JCjCd50HrRG7X0n7ExXFkvuZo2Yp2fBEyLSpPPh01INlEKU7vXE8SPXY5SPgzOo2
  ...
  Scenario: Updating the latest Nextcloud 19.0.3 on the beta channel
    Given There is a release with channel "beta"
    And The received version is "19.0.3.1"
    And The received PHP version is "7.2.0"
    And the installation mtime is "11"
    When The request is sent
    Then The response is non-empty
    And Update to version "20.0.0.8" is available
    And URL to download is "https://download.nextcloud.com/server/prereleases/nextcloud-20.0.0RC2.zip"
    And URL to documentation is "https://docs.nextcloud.com/server/20/admin_manual/maintenance/upgrade.html"
    And EOL is set to "0"
    And The signature is
    """
    h6VP+r1e3EcZF0MQn0h7jXJjI/5t87G5SH5GRNjJrrSq2ADPi38gJ0b0gGJqF+u4
