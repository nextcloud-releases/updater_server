Feature: Testing the update scenario of beta releases
  ...
  Scenario: Updating an outdated Nextcloud 18.0.14.1 on the beta channel
    ...
    And URL to download is "https://download.nextcloud.com/server/prereleases/nextcloud-19.0.10RC1.zip"
  ...
  Scenario: Updating the latest Nextcloud 19.0.10 on the beta channel
    ...
    When The request is sent
    Then The response is non-empty
    And Update to version "20.0.9.0" is available
    And URL to download is "https://download.nextcloud.com/server/prereleases/nextcloud-20.0.9RC1.zip"
    And URL to documentation is "https://docs.nextcloud.com/server/20/admin_manual/maintenance/upgrade.html"
    And EOL is set to "0"
    And The signature is
    """
    BTq4W7pfI6BUSbZuCywrGSLWM6y++oZ1NmKvKAscVgYatLLttpk4GtzDRRzt0pJ7
  ...
  Scenario: Updating the Nextcloud 20.0.9 on the beta channel
    Given There is a release with channel "beta"
    ...
    When The request is sent
    Then The response is non-empty
    And Update to version "21.0.1.0" is available
    And URL to download is "https://download.nextcloud.com/server/prereleases/nextcloud-21.0.1RC1.zip"
    And URL to documentation is "https://docs.nextcloud.com/server/21/admin_manual/maintenance/upgrade.html"
    And EOL is set to "0"
    And The signature is
    """
    K0Fftww7j+EyvV/OAMJB1ycKI4JtLTFINMCsGrWpdQuiiqbQdWQvUhOzRK08EcXL
